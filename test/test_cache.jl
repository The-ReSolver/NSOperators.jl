...
@testset "Cache pressure update         " begin
    ...
    P = copy(NSOperators.P(cache))
    ΔP = d2dy2!(P, SpectralField(grid)) .+ d2dz2!(P, SpectralField(grid))

    # initialise rhs field
    # ? the laplacian of pressure and rhs nonlinear terms do not individually match (at mean spanwise mode), but collectively (their difference) they do. why??? ?
    # rhs_fun(y, z, t) = 2*(π^2)*(sin(t)^2)*(cos(π*y)*(cos(π*y) + 1)*(sin(z)^2) - (sin(π*y)^2)*(cos(z)^2)) - Ro*(π*cos(π*y)*exp(cos(z))*sin(t) + dūdy_fun(y))
    rhs_fun(y, z, t) = -Ro*(π*cos(π*y)*exp(cos(z))*sin(t) + dūdy_fun(y))
    rhs = FFT!(SpectralField(grid), PhysicalField(grid, rhs_fun))
    ...
    Nz = 8
    Nt = 8
    display(round.(ΔP[Ny, 1:Nz, 1:Nt]; digits=5))
    println()
    display(round.(rhs[Ny, 1:Nz, 1:Nt]; digits=5))
    println()

    display(round.(cache.spec_cache[17][1, 1:Nz, 1:Nt]; digits=5))
    println()
    display(round.(cache.spec_cache[17][end, 1:Nz, 1:Nt]; digits=5))
    println()
