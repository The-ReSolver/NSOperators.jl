...
@testset "Local residual calculation    " begin
    ...
    rx_fun_no_p(y, z, t) = 0.0
    ry_fun_no_p(y, z, t) = 0.0
    rz_fun_no_p(y, z, t) = 0.0
    ...
    display(round.(parent(res_calc_phys[2]), digits=5)[2, :, :])
    println()
    display(round.(parent(res_phys[2]), digits=5)[2, :, :])
    println()
    # @test res_calc_phys ≈ res_phys rtol=1e-1

    using Plots
    using Printf
    for nt in 2:2:16
        @views begin
            p1 = contour(points(grid)[2], sort(points(grid)[1]), res_phys[1][:, :, nt])
            p2 = contour(points(grid)[2], sort(points(grid)[1]), res_calc_phys[1][:, :, nt])
        end
        plot(p1, p2, layour=(1, 2), legend=false)
        savefig("./plots/lrx_Nt=$(string(nt)).pdf")
    end
end
...
#     using Printf
#     res_calc_phys = VectorField(grid; field_type=:Physical)
#     IFFT!(res_phys, res_spec)
#     IFFT!(res_calc_phys, res_calc)
#     for nt in 1:Nt
#         @views begin
#             p1 = contour(points(grid)[2], sort(points(grid)[1]), res_phys[1][:, :, nt])
#             p2 = contour(points(grid)[2], sort(points(grid)[1]), res_calc_phys[1][:, :, nt])
