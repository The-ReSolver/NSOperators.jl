...
function dℜ_fd!(U::V, cache::Cache{T, S}, grad_fd::V; step::Float64=1e-6, comp::Int=1) where {T, S, V<:AbstractVector{S}}
    # loop over velocity field
    # for i in 1:size(U)[1], nt in 1:size(U[1])[3], nz in 1:size(U[1])[2], ny in 1:size(U[1])[1]
    for i in comp, nt in 1:size(U[1])[3], nz in 1:size(U[1])[2], ny in 1:size(U[1])[1]
        ...
        Ur_for[i][ny, nz, nt] = U[i][ny, nz, nt] + step*abs()
        update_v!(Ur_for, cache)
        update_p!(cache)
        localresidual!(Ur_for, cache)
        gr_for_r = ℜ(cache)
        Ur_back[i][ny, nz, nt] = U[i][ny, nz, nt] - step*abs()
        ...
        Ui_for[i][ny, nz, nt] = U[i][ny, nz, nt] + 1im*step*abs()
        update_v!(Ui_for, cache)
        update_p!(cache)
        localresidual!(Ui_for, cache)
        gr_for_i = ℜ(cache)
        Ui_back[i][ny, nz, nt] = U[i][ny, nz, nt] - 1im*step*abs()
...
function dℜ_fd2!(U::V, cache::Cache{T, S}, grad_fd::V; step::Float64=1e-6) where {T, S, V<:AbstractVector{S}}
    function inner_ℜ(U; _cache=cache)
        update_v!(U, _cache)
        update_p!(_cache)
        localresidual!(U, _cache)
        return ℜ(_cache)
    end

    grad_fd .= grad(central_fdm(4, 1), inner_ℜ, U)
end
